...
suite of \dfmtodo{XXXX} injection and recovery tests, five tests per
target star.
...
eclipse and these planet candidates are listed in \tab{cand}.
Two were previously published \citep{Crossfield:2015} but, to our knowledge,
...
SOMEFIGURE shows the observed distribution of planet candidates in our
catalog.
...
This function is shown in SOMEFIGURE and IS THIS CONSISTENT INTERESTING ETC?

In \papertwo, we will characterize the stars for each of our candidates in
detail but for now it's worth noting that about half (IS THIS TRUE) of our
planet candidates are orbiting M-type stars.
If this rate remains robust after stellar characterization and if these
numbers are representative of the \KT\ yields in upcoming Campaigns, this
