...
planet candidates transiting 31 stars, along with a completeness analysis
based on artificial signal injections.
...
We also present a list of 18 candidate eclipsing binary systems.
...
orbiting 31 stars---and a supplementary list of 18 eclipsing binary
...
Many of these signals are due to astrophysical ``false positives'' such as
eclipsing binary systems, either as the target star or as a background ``blend.''
...
that contribute only a small fraction of the total light in a target
star's photometric aperture, thus display greatly diluted eclipse
depths able to mimic the signals of small planets.  
...
false positive rate \citep{Morton:2011, Fressin:2013}, primarily due
to two major factors.  First, the superior precision of
the \kepler\ photometry enabled 
...
planet candidates are orbiting stars selected for \KT\ as M-type stars.
