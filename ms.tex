...
The vast majority of these signals are not induced by a
transiting planet; there are many false positives.
...
each of the \dfmtodo{YYYY} candidates \emph{by hand} to discard signals that are not
convincing transits.
The results of this vetting can be seen online at \dfmtodo{SOMEURL}.

This vetting resulted in a list of \dfmtodo{ZZZZ} transiting candidates.
...
robustly reject many candidates but of the list of \dfmtodo{100 (RIGHT NUMBER)}, two
candidates---\dfmtodo{EPIC AAA and BBB}---have offsets large enough that we remove them
from our final catalog.
\dfmtodo{Figure SOMEFIGURE} shows the distribution of centroid offsets across the full
catalog.
