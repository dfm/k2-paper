...
\dfmtodo{Iterative search.}


\paragraph{Initial candidate list}

To generate an initial candidate list, we estimate the signal-to-noise of
each candidate in the \TIC\ periodogram by comparing the peak height to a
robust estimate of variance in \TIC\ values across period.
A cut in this quantity can reject most variable stars and low
signal-to-noise candidates that can't be reliably recovered from the data.
...
This might be due to the fact that our independence assumption (SOMEEQUATION)
